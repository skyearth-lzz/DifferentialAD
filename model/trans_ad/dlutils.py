...
class PositionalEncoding(nn.Module):
    def __init__(self, d_model, dropout=0.1, max_len=5000):
        ...
        # 最后的位置编码矩阵大小为 max_len * d_model
        pe = torch.zeros(max_len, d_model)
        # position的维度为 max_len * 1
        position = torch.arange(0, max_len, dtype=torch.float).unsqueeze(1)

        # div_term的维度为 1 * d_model
        div_term = torch.exp(torch.arange(0, d_model).float() * (-math.log(10000.0) / d_model))
        pe += torch.sin(position * div_term)
        pe += torch.cos(position * div_term)
        # pe维度变为max_len * 1 * d_model, 是为了方便与输入张量进行批处理操作。
        pe = pe.unsqueeze(0).transpose(0, 1)
        # 这个方法用于将pe注册为模型的一个缓冲区。缓冲区是模型的一部分，但不会在训练过程中更新（即不会计算梯度）。
        # 这对于存储像位置编码这样的固定数据非常有用，因为它们不需要在训练过程中进行优化。
...
from model.trans_ad.multihead_diffattn import MultiheadDiffAttn

class TransformerEncoderLayer(nn.Module):
    def __init__(self, d_model, nhead, dim_feedforward=16, dropout=0):
        super(TransformerEncoderLayer, self).__init__()
        # self.self_attn = nn.MultiheadAttention(d_model, nhead, dropout=dropout)
        self.self_attn = MultiheadDiffAttn(d_model, nhead, dropout=dropout)
...
class TransformerDecoderLayer(nn.Module):
    def __init__(self, d_model, nhead, dim_feedforward=16, dropout=0):
        ...
        self.multihead_attn = MultiheadDiffAttn(d_model, nhead, dropout=dropout)
        self.linear1 = nn.Linear(d_model, dim_feedforward)
        self.dropout = nn.Dropout(dropout)
        self.linear2 = nn.Linear(dim_feedforward, d_model)
        self.dropout1 = nn.Dropout(dropout)
